...
export default function ManagerDashboardPage() {
  ...
  /* ── reusable blobs + arrow ── */
  const Blobs = () => (
    <>
      <span aria-hidden className="pointer-events-none absolute -right-6 -top-6 h-28 w-28 rounded-full bg-white/10 blur-2xl transition-all duration-500 group-hover:scale-150" />
      <span aria-hidden className="pointer-events-none absolute -bottom-8 -left-8 h-32 w-32 rounded-full bg-white/10 blur-3xl transition-all duration-500 group-hover:scale-125" />
    </>
  );
  const ArrowIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 transition-transform duration-300 group-hover:translate-x-1">
  ...
  return (
    <DashboardShell role="manager">
      ...
      <div className="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {/* Total Clients → workers page (managers manage workers/clients) */}
        <a
        ...
        >
          ...
          <p className="text-3xl font-bold text-white drop-shadow-sm">
            {loading ? "—" : workers.length.toLocaleString()}
          </p>
